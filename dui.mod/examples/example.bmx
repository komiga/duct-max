...
Type MyGraphicsApp Extends dProtogGraphicsApp
	...
	Method OnInit()
		...
		dProtogDrawState.InitDefaultState()
	...
	Method InitResources()
		Try
			m_defaultfont = New dProtogFont.FromNode(New dScriptFormatter.LoadFromFile("themes/fonts/arial.font"), True)
		...
		End Try
		Try
			Local theme:duiTheme = duiThemeManager.AddThemeFromFile("themes/theme_generic.theme", True, False)
			If theme
	...
	Method Update()
		TFPSCounter.Update()
		m_fps_label.SetText("FPS:" + TFPSCounter.GetFPS() + "; " + m_textfield.m_cursorpos)
	...
	Function CanvasDraw(x:Float, y:Float, canvas:duiCanvas)
		Local dtext:String = "Foooooobaaarrr"
		dProtog2DDriver.SetBlend(BLEND_ALPHA)
		dProtogDrawState.m_defaultstate.SetStates(True, True, False, False, False, False)
		x:+ 5.0; y:+ 70.0
		For Local i:Int = 0 Until dtext.Length
			duiFontManager.RenderString(dtext[i..i + 1], canvas.m_font, x, y - Cos(MilliSecs()) * 10.0)
			y:+ Sin(MilliSecs()) * 0.75
			x:+ duiFontManager.StringWidth(dtext[i..i + 1], canvas.m_font)
		Next
	End Function
