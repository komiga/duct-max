...
Rem
	table.bmx (Contains: duiTable, duiTableItem, )
End Rem
			...
			If m_header = True
				If m_background[1] = True
					For index = 0 To m_heading.Length - 1
						dProtogPrimitives.DrawRectangleToSize(rx + tx, ry, m_columnwidths[index], m_itemheight)
				...
				' Add the text
				For index = 0 To m_heading.Length - 1
					duiFontManager.RenderString(m_heading[index], m_font, rx + tx, ry + 3)
					tx:+3 + m_columnwidths[index]
				Next
			Else
				ry:-(m_itemheight + 2)
			...
			For item = EachIn m_items
				...
				If m_background[0] = True Or itemcount - 1 = m_selected
					...
					For index = 0 To m_heading.Length - 1
						If dui_IsInViewport(rx + tx, ry + (itemcount * (m_itemheight + 2)) + m_oldy, m_columnwidths[index], m_itemheight)
							dProtogPrimitives.DrawRectangleToSize(rx + tx, ry + (itemcount * (m_itemheight + 2)) + m_oldy, m_columnwidths[index], m_itemheight, True)
						End If
				...
				For index = 0 To m_heading.Length - 1
					If dui_IsInViewport((rx + tx) - 3, ry + (itemcount * (m_itemheight + 2)) + m_oldy, m_columnwidths[index], m_itemheight)
						duiFontManager.RenderString(item.GetContentAtIndex(index), m_font, rx + tx, ry + 3 + (itemcount * (m_itemheight + 2)) + m_oldy)
					End If
					tx:+3 + m_columnwidths[index]
				Next
			Next
...
'#region Collections
	...
	Method AddColumn(_heading:String, _width:Int = 0, dorefresh:Int = True)
		...
		If m_columnwidths[nlen - 1] < (duiFontManager.StringWidth(_heading, GetFont()) + 6)
			SetColumnWidth(nlen - 1, duiFontManager.StringWidth(_heading, GetFont()) + 6)
		End If
	...
	Method AddItemByData:duiTableItem(text:String[], data:Int = 0, extra:Object = Null, dorefresh:Int = True)
		...
		item = New duiTableItem.Create(text, data, extra)
	...
	End Rem
	Method RemoveItem:Int(_item:duiTableItem)
		If _item <> Null
	...
	Method FindData:Int(_data:Int, _start:Int = 0)
		Local item:duiTableItem, index:Int
