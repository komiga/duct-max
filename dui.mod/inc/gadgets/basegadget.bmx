...
Rem
	basegadget.bmx (Contains: duiGadget, )
End Rem
...
'#region Gadget function
	...
	Method BindDrawingState(index:Int = 0)
		If index > - 1 And index < m_color.Length
			dProtog2DDriver.BindPColor(m_color[index], True)
		End If
	End Method
	...
	Method BindTextDrawingState(index:Int = 0)
		If index > - 1 And index < m_textcolor.Length
			dProtog2DDriver.BindPColor(m_textcolor[index], True)
		End If
	End Method
...
'#region Collections
	...
	Rem
		...
		returns: A duiGadget or Null if the gadget by the given name was not found.
	End Rem
	Method GetChildByName:duiGadget(name:String)
		If name <> Null
			name = name.ToLower()
