...
Rem
	themerenderer.bmx (Contains: duiThemeManager, duiThemeStub, duiTheme, duiThemeSection, duiThemeSectionSet, )
End Rem
...
'#region Collections
	...
	End Rem
	Function GetThemeFromName:duiTheme(name:String)
		Return duiTheme(m_themes._ValueByKey(name))
	End Function
	...
	End Rem
	Function AddThemeFromObject:duiTheme(url:Object, loadtexture:Int = True, binary:Int = False, encoding:Int = SNPEncoding_UTF8)
	...
	Rem
		...
		about: If the given url is a binary theme (rather than a script), pass @binary as True (defaults to False - script).
	End Rem
	Function AddThemeFromStream:duiTheme(stream:TStream, loadtexture:Int = True, binary:Int = False, encoding:Int = SNPEncoding_UTF8)
		Local theme:duiTheme
		If stream <> Null
			If binary = True
				theme = New duiTheme.Deserialize(stream)
			Else
				Local node:dSNode = dSNode.LoadScriptFromObject(stream, encoding)
				If node <> Null
...
'#region Collections
	...
	Rem
		...
		returns: The #duiThemeStub from the name given, or Null if there is no stub with the name given.
	End Rem
	Method _GetStubFromName:duiThemeStub(name:String)
		Return duiThemeStub(_ValueByKey(name))
	End Method
...
'#end region (Data handlers)
	...
	Method Report:String(tab:String = Null)
		Local stub:duiThemeStub, str:String
...
'#region Collections
	...
	Rem
		...
		returns: Nothing.
	End Rem
	Method AddSectionSetsFromNode(root:dSNode)
		Local node:dSNode, set:duiThemeSectionSet
	...
	End Rem
	Method GetSectionSetFromName:duiThemeSectionSet(name:String)
		Return duiThemeSectionSet(_GetStubFromName(name))
	End Method
...
'#region Data handlers
	...
	End Rem
	Method FromNode:duiTheme(root:dSNode)
	...
	End Rem
	Method ToNode:dSNode(nodename:String = "")
		Local node:dSNode, iden:dIdentifier
		Local node_sets:dSNode, set:duiThemeSectionSet
		
		node = New dSNode.Create(nodename)
		iden = New dIdentifier.CreateByData(m_template_name.GetIden()[0])
		iden.AddValue(New dStringVariable.Create(Null, m_name))
		node.AddIdentifier(iden)
		
		iden = New dIdentifier.CreateByData(m_template_formalname.GetIden()[0])
		iden.AddValue(New dStringVariable.Create(Null, m_formalname))
		node.AddIdentifier(iden)
		
		iden = New dIdentifier.CreateByData(m_template_textureurl.GetIden()[0])
		iden.AddValue(New dStringVariable.Create(Null, m_textureurl))
		node.AddIdentifier(iden)
		
		node_sets = New dSNode.Create("sets")
		For set = EachIn ValueEnumerator()
	...
	Method Copy:duiTheme()
		Local clone:duiTheme, set:duiThemeSectionSet
		
		clone = New duiTheme.Create(m_name, m_formalname, m_textureurl)
		For set = EachIn ValueEnumerator()
...
Type duiThemeSectionSet Extends duiThemeStub
	...
	Method ImportSet(sectionset:duiThemeSectionSet)
		Local stub:Object, set:duiThemeSectionSet, section:duiThemeSection
...
'#region Update & miscellaneous
	...
	Method Update(theme:duiTheme)
		Local child:Object, set:duiThemeSectionSet, section:duiThemeSection
...
'#region Collections
	...
	Rem
		...
		returns: The #duiThemeSection with the name given, or Null if no section by the given name exists.
	End Rem
	Method GetSectionFromName:duiThemeSection(name:String)
		Return duiThemeSection(_GetStubFromName(name))
	End Method
	Rem
		bbdoc: Get a section from the given structure.
		returns: The #duiThemeSection from the given structure, or Null if no section was found at the end of the structure.
...
'#region Data handlers
	...
	End Rem
	Method FromNode:duiThemeSectionSet(root:dSNode)
		Local child:Object, iden:dIdentifier, node:dSNode
		Local set:duiThemeSectionSet
		...
		For child = EachIn root.GetChildren()
			...
			If iden <> Null
				...
				If m_template_importsection.ValidateIdentifier(iden) = True
					Local structure:String, theme:duiTheme, section:duiThemeSection
					
					structure = iden.GetValueAtIndex(0).ValueAsString()
					theme = duiTheme(GetTopParent())
					If theme <> Null
				...
				Else If m_template_importset.ValidateIdentifier(iden) = True
					Local structure:String, theme:duiTheme
					...
					End If
				Else If duiThemeSection.ValidateIdentifier(iden) = True
	...
	End Rem
	Method ToNode:dSNode()
		Local stub:duiThemeStub, node:dSNode
		
		For stub = EachIn ValueEnumerator()
			If duiThemeSection(stub)
				node.AddIdentifier(duiThemeSection(stub).ToIdentifier())
			Else If duiThemeSectionSet(stub)
				node.AddNode(duiThemeSectionSet(stub).ToNode())
			End If
		Next
		Return node
	End Method
	...
	Method Copy:duiThemeSectionSet()
		Local clone:duiThemeSectionSet
		Local stub:Object, set:duiThemeSectionSet, section:duiThemeSection
		
		clone = New duiThemeSectionSet.Create(m_name)
		For stub = EachIn ValueEnumerator()
			set = duiThemeSectionSet(stub)
			section = duiThemeSection(stub)
			If set <> Null
...
'#region Data handlers
	...
	Method FromIdentifier:duiThemeSection(identifier:dIdentifier)
		...
		Local variable:dVariable = identifier.GetValueAtIndex(1)
		If variable <> Null
	...
	Method ToIdentifier:dIdentifier()
		Local identifier:dIdentifier
		
		identifier = New dIdentifier.CreateByData(m_template_section.GetIden()[0])
		identifier.AddValue(New dStringVariable.Create(Null, m_name))
		If m_x <> 0.0 Or m_y <> 0.0 Or m_width <> 0.0 Or m_height <> 0.0
			identifier.AddValue(New dFloatVariable.Create(Null, m_x))
			identifier.AddValue(New dFloatVariable.Create(Null, m_y))
			If m_width <> 0.0 Or m_height <> 0.0
				identifier.AddValue(New dFloatVariable.Create(Null, m_width))
				identifier.AddValue(New dFloatVariable.Create(Null, m_height))
			End If
		End If
		Return identifier
	End Method
	...
	Method Copy:duiThemeSection()
		Local clone:duiThemeSection
		clone = New duiThemeSection.Create(m_name, m_x, m_y, m_width, m_height)
		Return clone
	End Method
