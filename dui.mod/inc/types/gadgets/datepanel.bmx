...
Rem
	datepanel.bmx (Contains: dui_DatePanel, )
End Rem

Rem
	bbdoc: The dui date panel gadget type.
End Rem
Type dui_DatePanel Extends dui_Gadget
	
	Global m_renderer:dui_GenericRenderer = New dui_GenericRenderer
	
	Field m_date:dui_Date
	Field m_month:dui_ComboBox
	Field m_year:dui_ComboBox
	Field m_calender:dui_Table
	...
	End Rem
	Method Create:dui_DatePanel(name:String, w:Float, h:Float, starty:Int, endy:Int, date:dui_Date)
		Local index:Int
		...
		m_date = date
		m_month = New dui_ComboBox.Create(name + ":MonthCombobox", "", 5.0, 5.0, 95.0, 20.0, 95.0, 270.0, 18.0, Self)
		For index = 1 To 12
			m_month.AddItemByData(dui_MonthAsString(index, True), index,, False)
		Next
		
		m_year = New dui_ComboBox.Create(name + ":YearComboBox", "", 105.0, 5.0, 70.0, 20.0, 80.0, 200.0, 18.0, Self)
		m_calender = New dui_Table.Create(name + ":ResultsTable", 5.0, 30.0, 145.0, "S", 22.0, 18.0, True, Self)
...
'#region Gadget function
	...
	Method Deactivate()
		...
		New dui_Event.Create(dui_EVENT_GADGETCLOSE, m_date, 0, 0, 0, Null)
	End Method
