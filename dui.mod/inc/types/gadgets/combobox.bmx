...
Rem
	combobox.bmx (Contains: dui_ComboBox, )
End Rem

Rem
	bbdoc: The dui combobox gadget type.
End Rem
Type dui_ComboBox Extends dui_Gadget
	
	Global m_renderer:dui_GenericRenderer = New dui_GenericRenderer
	
	Field m_defaulttext:String, m_text:String
	Field m_menu:dui_Menu
	
	Rem
		bbdoc: Create a combobox.
		returns: The created combobox (itself).
	End Rem
	Method Create:dui_ComboBox(name:String, text:String, x:Float, y:Float, w:Float, h:Float, menuwidth:Float, menuheight:Float, itemheight:Float, parent:dui_Gadget)
		...
		m_menu = New dui_Menu.Create(name + ":Menu", x, y + h + 2.0, menuwidth, menuheight, itemheight)
...
'#region Render & update methods
	...
	Method UpdateMouseRelease(x:Int, y:Int)
		...
		If dui_MouseIn(m_x + x, m_y + y, m_width, m_height) = True
			...
			ay = rely + m_height + 2
			If ay + m_menu.m_height > TDUIMain.GetScreenHeight()
	...
	Method UpdateText()
		...
		New dui_Event.Create(dui_EVENT_GADGETSELECT, Self, GetSelectedItemData(), 0, GetSelectedItemIndex(), GetSelectedItemExtra())
	End Method
...
'#region Collections
	...
	Method ClearItems()
		...
		'New dui_Event.Create(dui_EVENT_GADGETSELECT, Self, GetSelectedItemData(), 0, GetSelectedItemIndex(), Null)
	End Method
...
'#region Gadget function
	...
	Method Open(x:Float, y:Float)
		m_menu.Activate(x, y)
		New dui_Event.Create(dui_EVENT_GADGETOPEN, Self, 1, 0, 0, m_menu)
	End Method
