...
float blurfactors[15] = float[15]	(0.0044, 0.0115,
									0.0257, 0.0488,
									0.0799, 0.1133,
									0.1394,
...
void main() {
	vec4 color = vec4(0.0);
	vec2 viewport = p2d_viewportsize;
	viewport -= vec2(2.0, 2.0);
	for(int i = 0; i < 15; ++i) {
		vec2 tc = min(gl_TexCoord[0].st + blurscale * float(i-7), viewport);
